...
for df,df_str in zip(df_list,['df_128']): # for 64x64
	...
	hbkg1 = plt.hist(df_bkg['prediction'], histtype='step', stacked=True,weights=1000*lumiVal*df_bkg['weight'], bins=151684, alpha=0.7, color=color_map_QCD[df_str], label='QCD_%s' %(label_name))
	plt.yscale('log')
...
for df,df_str in zip(df_list,['df_128']): #for 64x64
	...
	label_name = 'N' + df_str.split('_')[1]
	hsig1 = plt.hist(df_sig['prediction'], histtype='step', stacked=True,weights=1000*lumiVal*df_sig['weight'], bins=151684, alpha=0.7, color=color_map_SIG[df_str], label='RPV_%s' %(label_name))
...
for hsig,hbkg in zip(hsig_arr,hbkg_arr):
	...
	Score = list([round(i*0.001,2) for i in range(0,1000)])
	#Score = list([round(i*6.592653147332613e-06,1) for i in range(0,151684)])
	...
	for cut in range(0,len(Score),1):
		...
		significance = sig_integral / math.sqrt(sig_integral+bkg_integral) # Rouch 
